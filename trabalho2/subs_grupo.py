...
def gform(cname='', submenu=""):
    ...
    if ulogin is not None:
        ...
        if prev_option == 'insert' and option == 'save':
            ...
            idviagem = request.form['_viagemid']
            if idviagem in Viagem.obj:
                obj = cl.from_string(strobj)
                cl.insert(getattr(obj, cl.att[0]))
                cl.last()
            else:
                viagem_not_found = True
        ...
        if cname == 'Grupo':
            return render_template("grupo.html", butshow=butshow, butedit=butedit,
                                   cname=cname, obj=obj_dict, att=cl.att, header=cl.header, des=cl.des,
                                   ulogin=session.get("user"), auto_number=cl.auto_number,
                                   submenu=submenu, viagem_not_found=viagem_not_found)
        else:
